...
// This function handles all the incoming and outgoing communication. 
// To insert an event into the network or in the device's observation, it suffices to add the event in the correspondig queues. To erase
// an event that was received by the device, one has to remove it from the queue of incoming events and not call the method triggerIfPossible.
// To erase an event before it is sent into the network, one has to remove it from the queue of outgoing events and not send the CAN package.
